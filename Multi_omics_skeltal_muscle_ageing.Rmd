...
gtex_tisue <- read.table("/mnt/vol1/enrichment/ske/gene_reads_2017-06-05_v8_muscle_skeletal.gct.gz", header = TRUE, sep = "\t", skip = 2)

gene_info <- gtex_tisue[, 1:2]
expr_counts <- gtex_tisue[, -c(1, 2)]  
expr_counts <- as.matrix(sapply(expr_counts, as.numeric))

rownames(expr_counts) <- gene_info$Description
...
## Encode age (use ordinal code instead of midpoints if you prefer)
pheno_muscle2 <- pheno_muscle2 %>%
  mutate(
    AGE_ord = as.numeric(factor(
      AGE,
      levels  = c("20-29","30-39","40-49","50-59","60-69","70-79"),
      ordered = TRUE
    ))
  ) %>%
  filter(!is.na(AGE_ord))

# Align expression 
expr_counts <- expr_counts[, pheno_muscle2$SAMPID]
rownames(pheno_muscle2) <- pheno_muscle2$SAMPID
pheno_muscle2$AGE_ord <- as.numeric(pheno_muscle2$AGE_ord)

dds <- DESeqDataSetFromMatrix(
  countData = expr_counts,    
  colData   = pheno_muscle2,
  design    = ~ AGE_ord
)
...
rna_gene <- as.data.frame(res_age) %>%
  ...
  transmute(
    gene,
    beta_age_rna = log2FoldChange,   # change per age band
    p_rna        = pvalue,
    FDR_rna      = padj
  )
